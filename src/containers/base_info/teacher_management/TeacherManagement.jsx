...
import styles from './TeacherManagement.scss'
...
const TeacherManagement = React.createClass({
  ...
  getTableData(){
    ...
    tableHeader = fromJS([{
      ...
      className:styles.tableColumn,
      render:(text,record)=>{
        return (<a><Icon type='edit'/>{`班级数量:${text}`}</a>)
      }
    }])
    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          {this._currentMenu.get('authList').some(v => v.get('authUrl')=='/grade/add')?<Button type="primary" style={{backgroundColor:'#FD9B09',borderColor:'#FD9B09'}} onClick={()=>{this.setState({showAddGradeModal:true})}}>新建</Button>:<div> </div>}<Search style={{width: '200px'}} placeholder="请输入查询条件" value={this.state.searchStr} onChange={(e)=>{this.setState({searchStr:e.target.value})}} onSearch={this.handleSearchTableData} />
        </div>
...
export default connect(mapStateToProps,mapDispatchToProps)(Form.create()(TeacherManagement))
