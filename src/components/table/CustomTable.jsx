...
export class CustomTable extends React.Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    if (this.props.pageUrl != nextProps.pageUrl) {
      ...
      nextState.result = [];
      const param = Map(nextProps.additionalParams).concat({
        currentPage: 1,
  ...
  render() {
    ...
    const showFilter = (filters && filters.size > 0);
    const showAction = (actions && actions.size > 0);
            ...
            {showFilter ? <div className={styles.filterItems}>
                {
                  filters.map(function (filter) {
                    if (filter.type == "select") {
                      return <Select key={filter.key} defaultValue="" style={{marginLeft: 10}}>
  ...
  searchTableData() {
    const param = Map(this.props.additionalParams).concat({
